...
TEST_F(NioControllerTest, Status) {
  controller_.Init(params_, &sender_, &msg_manager_);
  ...
  controller_.EpsReqC6_->set_epsdaireq(10.5);
  EXPECT_FALSE(controller_.CheckChassisError());
}

TEST_F(NioControllerTest, UpdateDrivingMode) {
  controller_.Init(params_, &sender_, &msg_manager_);
