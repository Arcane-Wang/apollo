...
class VehicleController {
 ...
 private:
  ...
  virtual void setAcTemperature(const control::ControlCommand &command){};
  virtual void setAcMode(const control::ControlCommand &command){};
  virtual void setAcWindSpeed(const control::ControlCommand &command){};
  virtual void setAmbientLamps(const control::ControlCommand &command){};
  virtual void setInnerCeilingLamps(const control::ControlCommand &command){};
  virtual void setFogLamps(const control::ControlCommand &command){};
  virtual void setPositionLamps(const control::ControlCommand &command){};
  virtual void setDrivingLamps(const control::ControlCommand &command){};
  virtual void setDoor(const control::ControlCommand &command){};

 public:
  void UpdateLastTimestampInControlMessage(void);
  void SaveControlMessageState(const control::ControlCommand &contro_command,
...
//   void getDebug(IDebugChassis &debug);
 ...
 protected:
  ...
  virtual void set_chassis_error_code(const Chassis::ErrorCode &error_code) = 0;
 ...
 protected:
  ...
  std::atomic<uint64_t> last_timestamep_on_control_message_;
  uint64_t max_latency_;
  std::mutex debug_mutex_;
//   IDebugChassis debug_;
};
