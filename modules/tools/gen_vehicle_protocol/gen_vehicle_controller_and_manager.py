...
def gen_vehicle_controller_header(content, output_dir):
    ...
    with open(controller_header_file, "w") as header:
        ...
        control_protocol_include_fmt = (
            '#include "modules/canbus/vehicle/%s/protocol/%s.h"'
        )
        ...
        for pid in protocols:
            ...
            if p["protocol_type"] == "control":
                name = p["name"]
                include = control_protocol_include_fmt % (
                    car_type.lower(),
                    bf.convert_upper_to_underline(name).lower(),
                )
...
def gen_message_manager_cpp(content, output_dir):
    ...
    with open(message_manager_cpp_tpl_file, "r") as tpl:
        fmt = tpl.readlines()
    car_type = content["car_type"]
    message_manager_cpp_file = output_dir + ("%s_message_manager.cc" % car_type.lower())
    with open(message_manager_cpp_file, "w") as cpp:
        ...
        for p_name in protocols:
            p = protocols[p_name]
            var_name = "%s" % bf.convert_upper_to_underline(p["name"])
            class_name = p["name"].replace("_", "").capitalize()
            header = header_fmt % (car_type.lower(), var_name.capitalize().replace("_", ""))
            if p["protocol_type"] == "control":
...
def gen_vehicle_factory_cpp(content, output_dir):
    ...
    with open(vehicle_factory_cpp_tpl_file, "r") as tpl:
        fmt = tpl.readlines()
    car_type = content["car_type"]
    vehicle_factory_cpp_file = output_dir + ("%s_vehicle_factory.cc" % car_type.lower())
